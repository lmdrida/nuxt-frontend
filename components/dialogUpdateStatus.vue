      ...
      >
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            <h2 class="text-2xl font-medium mb-6">Update Status Activity</h2>
            <select v-model="selectedOption" class="text-md text-gray-700">
              <option v-for="(option, i) in listStatus" :key="i" :value="option.value">
                {{ option.text }}
              </option>
            </select>
          </div>
...
const selectedOption = ref(props.data.status);
...
const confirmDialog = () => {
  console.log("kaje");
  //   const res = await postStatus(selectedOption.value);
};
</script>
